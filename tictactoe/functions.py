"""Module pour jouer au TicTacToe en ligne de commande"""
import sys


class CaseDejaRemplie(Exception):
    """Exception levée quand un joueur essaye de remplir une case déjà remplie"""
...
class PartieEncoreEnCours(Exception):
    """Exception levée quand la propriété 'vainqueur' est accédée mais qu'il n'y a pas encore de vainqueur"""
...
\nPropriétés :
- vainqueur (Renvoi le nom du joueur gagnant s'il y en a un, sinon renvoie
 une Exception 'PartieEncoreEnCours')
- prochain_joueur (Renvoi le nom du joueur qui doit jouer le coup actuel)
\nExceptions :
- CaseDejaRemplie (Invoquée quand un joueur essaye de remplir une case déjà
 remplie)
- PartieEncoreEnCours (Invoquée quand la propriété 'vainqueur' est accédée mais qu'il n'y a pas encore de vainqueur)
    ...
    def __init__(self, player_1: Player, player_2: Player) -> None:
        self.__grille: list[list[str]] = [
            [self.SYMBOLE_VIDE for _ in range(3)] for _ in range(3)]
    ...
    def _verifinputgrille(f):
        ...
        def wrapper(*args):
            ...
            if args[0].grille[args[1]][args[2]] != args[0].SYMBOLE_VIDE:
    ...
    def __repr__(self) -> str:
        data_lines: list[str] = [
            "  " + "  |  ".join(cell for cell in ligne) + "  " for ligne in self.__grille]
        empty_line = "\n-----|-----|-----\n"
...
- 2 (la partie est PAT)"""
    ...
    def _check_etat_partie(self) -> None:
        ...
        colonnes: list[list[str]] = [[ligne[col_i] for ligne in self.__grille]
                                     for col_i in range(len(self.__grille))]
        ...
        diags: list[list[str]] = [[self.__grille[x][x] for x in range(len(self.__grille))], [
            self.__grille[x][y] for x, y in zip(range(len(self.__grille)), range(len(self.__grille) - 1, -1, -1))]]
        for diag in diags:
    ...
    def jouer(self, x: int, y: int) -> None:
        ...
        print(f"{self.__players[self.__turn %
              2].name} vient de jouer :\n{self}")
    ...
    def __eq__(self, etat: int | bool) -> bool:
        ...
        if not isinstance(etat, bool) and not isinstance(etat, int):
            raise TypeError(
                "La valeur de l'état doit être un entier ou un boolean")
        ...
        if isinstance(etat, int):
            if not 0 <= etat <= 2:
                raise ValueError(
                    "L'état ne peut avoir que 0, 1 ou 2 comme valeur")
...
if __name__ == "__main__":
    ...
    def jouer_partie(player_1: Player, player_2: Player) -> None:
        ...
        while not jeu:
            try:
                coup: list[int] = map(lambda x: int(x), input(
                    f"Que veux-tu jouer {jeu.prochain_joueur} ? (ligne, colonne) : ").replace(' ', '').split(","))
                clear_screen()
                jeu.jouer(*coup)
        ...
        def center_text(text, long): return (
            (long - len(text)) // 2) * " " + text + ((long - len(text)) // 2) * " "
        scores = f"{player_1.name} | {player_2.name}"
        scores_points = f"{center_text(str(player_1.points), len(
            player_1.name) + 1)}   {center_text(str(player_2.points), len(player_2.name) + 1)}"
        scores = f"\n{
            '-' * len(scores)}\n{scores}\n{scores_points}\n{'-' * len(scores)}"
        print(scores)
